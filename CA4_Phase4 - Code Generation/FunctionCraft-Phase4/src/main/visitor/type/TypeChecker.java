...
public class TypeChecker extends Visitor<Type> {
    public final ArrayList<String> visited = new ArrayList<>();
    public final HashMap<String, Type> funcReturnType = new HashMap<>();
    ...
    public Type visit(FunctionDeclaration functionDeclaration) {
        ...
        if (returnStmtsTypes.size() > 1) {
            ...
            SymbolTable.pop();
            Type returnType = returnStmtsTypes.iterator().next();
            funcReturnType.put(functionDeclaration.getFunctionName().getName(), returnType);
            try {
                ((FunctionItem) SymbolTable.root.getItem(FunctionItem.START_KEY +
                        functionDeclaration.getFunctionName().getName())).setReturnType(returnType);
            } catch (ItemNotFound ignored) {
            }
            return returnType;
        } else {
