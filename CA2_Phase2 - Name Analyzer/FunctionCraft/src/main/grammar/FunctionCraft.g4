...
patternMatching returns [PatternDeclaration patternRet]:
    ...
    RPAR
    {
        Identifier patId = new Identifier($patternName.text);
        patId.setLine($patternName.line);
        Identifier targId = new Identifier($targetVar.text);
        targId.setLine($targetVar.line);
        $patternRet = new PatternDeclaration(patId, targId);
        $patternRet.setLine($pat.line);
    }
    (PATTERN_MATCHING_SEPARATOR c = condition
    {
            ArrayList<Expression> currentConditions = $patternRet.getConditions();
            if (currentConditions == null) {
                currentConditions = new ArrayList<Expression>();
            }
            currentConditions.addAll($c.conditionRet);
            $patternRet.setConditions(currentConditions);
    }
     ASSIGN e = expression
     {
...
forStatement returns [ForStatement forStRet]:
    ...
    l = loopBody
    {
        Identifier forStmtId = new Identifier($id.text);
        forStmtId.setLine($id.line);
        $forStRet = new ForStatement(forStmtId, $r.rangeRet, $l.loopExps, $l.loopStmts, $l.loopRetStmt);
...
matchPatternStatement returns [MatchPatternStatement matchPatRet]:
    ...
    {
        Identifier patId = new Identifier($id.text);
        patId.setLine($id.line);
        $matchPatRet = new MatchPatternStatement(patId, $e.expRet);
